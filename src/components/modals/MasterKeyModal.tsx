...
import type { EnvironmentDetails, MasterKeyStatus } from "@/types/environment";
...
export default function MasterKeyModal({
  ...
  onClose,
  onSuccess,
}: Readonly<MasterKeyModalProps>) {
  // Setup / Unlock input
  const [keyInput, setKeyInput] = useState("");
  const [showKey, setShowKey] = useState(false);

  // Active / Manage state inputs
  ...
  // If workspace already has encrypted secrets that are locked, this is an Unlock flow.
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      // Re-read current active environment variables to decrypt secrets in UI
      const envName = activeEnvironmentName || "global";
      try {
        const details = await invoke<EnvironmentDetails>("read_environment", {
          name: envName,
        });
        setEnvVariables(envName, details.variables);
      } catch {
      ...
      notifications.show({
        ...
        message: hasMasterKey
          ...
          : "Master key loaded into RAM. Secret variables are now accessible.",
        color: "green",
      });
  ...
  const handleRemoveKey = async () => {
    ...
    try {
      ...
      notifications.show({
        ...
        message: "Master Key has been cleared from RAM.",
        color: "gray",
      });

      // Re-read environment to update locked states
      const envName = activeEnvironmentName || "global";
      try {
        const details = await invoke<EnvironmentDetails>("read_environment", {
          name: envName,
      ...
      <div className={classes.modalContent}>
        <div className={clsx(classes.banner, hasMasterKey && classes.bannerActive)}>
          ...
          <div className={classes.bannerText}>
            {hasMasterKey ? (
              <>
                Master Key is currently <strong>active in RAM</strong>. To clear it or rotate to a
                new key, please enter your current Master Key for security verification.
              </>
            ) : isUnlockMode ? (
              <>
                This workspace contains secret variables encrypted with <strong>AES-256-GCM</strong>
                . Enter the Master Key or Passphrase to unlock and load them into RAM for this
                session.
              </>
            ) : (
              <>
                ...
                <strong>strictly in RAM</strong> during this session and is never written to disk.
              </>
            )}
          </div>
        </div>
        ...
        {hasMasterKey ? (
          ...
          <div className={classes.inputGroup}>
            ...
            <TextInput
              ...
              placeholder={
                isUnlockMode
                  ? "Enter your workspace Master Key to unlock"
                  : "Enter passphrase or generate a 32-byte Base64 key"
              }
              className={classes.keyInput}
              rightSection={
                <Box style={{ display: "flex", gap: 2, marginRight: 4 }}>
                  {keyInput && !isUnlockMode && (
                    <Tooltip label={copied ? "Copied" : "Copy key"} position="top">
                      <ActionIcon
                        variant="subtle"
                        size="xs"
        ...
        <div className={classes.footer}>
          <div>
            {hasMasterKey && (
              <Button
                ...
                title={
                  !currentKeyInput.trim()
                    ? "Enter current Master Key to confirm clearing from RAM"
          ...
          <div className={classes.rightButtons}>
            ...
            >
              {hasMasterKey ? "Change Master Key" : isUnlockMode ? "Unlock" : "Set Master Key"}
            </Button>
          </div>
        </div>
      </div>
